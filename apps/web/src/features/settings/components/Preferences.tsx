...
export function PreferencesContent() {
    ...
    // Accumulates pending updates and debounces the Convex mutation.
    // Rapid clicks (e.g. cycling accent colors) are merged into a single write.
    ...
    const debouncedUpdateSettings = useCallback((updates: SettingsUpdates) => {
        ...
        // Clear any pending backend call
        if (debounceTimerRef.current) {
            clearTimeout(debounceTimerRef.current);
        }

        // Schedule Convex write after 300ms of inactivity
    ...
    // Accent CSS is now applied globally by useThemeInitializer in App.tsx.
    // No local effect needed here.
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="space-y-3">
                <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Data</h4>
                        ...
                        <div className="grid grid-cols-3 gap-1 bg-secondary/20 p-1 rounded-md">
                            {['light', 'dark', 'system'].map((t) => (
                                <button
                                    ...
                                    onClick={() => {
                                        setTheme(t);
                                        debouncedUpdateSettings({ theme: t as 'light' | 'dark' | 'system' });
                                    }}
                    ...
                    <div className="grid gap-1.5">
                        ...
                        <div className="grid grid-cols-2 gap-1 bg-secondary/20 p-1 rounded-md">
                            {[
                                ...
                                <button
                                    key={m.value}
                                    onClick={() => debouncedUpdateSettings({ colorMode: m.value as 'full' | 'monochromatic' })}
                    ...
                    <div className="grid gap-1.5">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-2">
                            {CURATED_ACCENTS.map(a => (
                                <button
                                    key={a.value}
                                    onClick={() => debouncedUpdateSettings({ systemAccent: a.value })}
