    ...
    const handleKeyDown = useCallback((e: KeyboardEvent) => {
        ...
        if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(e.key)) {
            ...
            idsToUpdate.forEach(id => {
                ...
                if (item) {
                    const newDate = addDays(new Date(item.date), days).getTime();
                    mutations.updateItem.mutate({
                        id: item.id,
                        updates: { date: newDate }
                    });
                } else if (milestone) {
                    const newDate = addDays(new Date(milestone.date), days).getTime();
                    mutations.updateMilestone.mutate({
                        id: milestone.id,
                        updates: { date: newDate }
                    });
                } else if (subProject) {
                    const newStart = addDays(new Date(subProject.startDate), days).getTime();
                    const newEnd = addDays(new Date(subProject.endDate), days).getTime();
