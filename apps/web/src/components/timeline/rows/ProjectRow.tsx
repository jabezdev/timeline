...
import { TimelineCell } from '../items/TimelineCell';
import { MilestoneItem } from '../items/MilestoneItem';
...
export const ProjectRow = memo(function ProjectRow({
  ...
  const { items, subProjectItems } = useMemo(() => {
    ...
    for (const t of propItems) {
      // Convert Unix ms to ISO date string for Map key lookup
      const tDateStr = format(new Date(t.date), 'yyyy-MM-dd');
      if (t.subProjectId) {
        ...
        if (!spMap.has(tDateStr)) spMap.set(tDateStr, []);
        spMap.get(tDateStr)!.push(t);
      } else {
        if (!items.has(tDateStr)) items.set(tDateStr, []);
        items.get(tDateStr)!.push(t);
      }
    }
