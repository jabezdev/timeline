...
export function useTimelineScroll(visibleDays: number = 21) {
    ...
    const setCurrentRange = useTimelineStore(state => state.setCurrentRange);
    ...
    // Update global store range whenever window changes
    useEffect(() => {
        const startTs = startDate.getTime();
        const endTs = addDays(startDate, visibleDays).getTime();
        setCurrentRange({ startDate: startTs, endDate: endTs });
    }, [startDate, visibleDays, setCurrentRange]);
