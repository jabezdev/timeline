...
export function PreferencesContent() {
    ...
    const debouncedUpdateSettings = useCallback((updates: Partial<{ systemAccent: string; colorMode: 'full' | 'monochromatic'; theme: 'light' | 'dark' | 'system' }>) => {
        ...
        // Apply optimistic cache update IMMEDIATELY (no waiting for debounce)
        queryClient.setQueryData(['timeline', 'structure'], (old: Partial<import('@/types/timeline').TimelineState> | undefined) => {
            if (!old) return old;
            return { ...old, userSettings: { ...old.userSettings, ...updates } };
        });
